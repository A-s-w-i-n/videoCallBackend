...
const rooms = new Map();
const users = new Map();
const connections = new Map(); // Map WebSocket connections to user IDs
...
function handleJoinRoom(ws, connectionId, { roomName, userName }) {
  ...
  sendToRoom(roomName, 'user-joined', { 
    userId: connectionId, 
    userName: userName,
    users: room.users
  }, ws);
...
function handleToggleVideo(ws, connectionId, { roomName, enabled }) {
  sendToRoom(roomName, 'user-video-toggle', { 
    userId: connectionId, 
    enabled 
  }, ws);
}
...
function handleToggleAudio(ws, connectionId, { roomName, enabled }) {
  sendToRoom(roomName, 'user-audio-toggle', { 
    userId: connectionId, 
    enabled 
  }, ws);
}
